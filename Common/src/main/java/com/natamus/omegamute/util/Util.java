...
public class Util {
	...
	private static boolean generateSoundFile() throws IOException {
		...
		Registry<SoundEvent> soundEventRegistry = Constants.mc.level.registryAccess().lookupOrThrow(Registries.SOUND_EVENT);

		List<String> soundNames = new ArrayList<String>();

		for (Identifier soundlocation : soundEventRegistry.keySet()) {
			Optional<Holder.Reference<SoundEvent>> optionalSoundEventReference = soundEventRegistry.get(soundlocation);
			optionalSoundEventReference.ifPresent(soundEventReference -> soundNames.add(soundEventReference.value().location().toString()));
		}
